...
const SQL_SNIPPET = `-- Run this in your Supabase SQL Editor (https://supabase.com/dashboard -> SQL Editor -> New Query):
...
console.log("==========================================");
console.log("Deadlnr DB Fix Script");
console.log("==========================================");
console.log("\nIf you are seeing error 23502 (null value in column 'user_id' violates not-null constraint),");
console.log("you need to run the following SQL script in your Supabase project.");
console.log("\nStep 1: Go to https://supabase.com/dashboard");
console.log("Step 2: Select your project");
console.log("Step 3: Go to SQL Editor in the left sidebar");
console.log("Step 4: Click 'New Query'");
console.log("Step 5: Copy and paste the script below, then click 'Run'\n");
console.log("------------------------------------------");
console.log(SQL_SNIPPET);
console.log("------------------------------------------");

console.log("\nAlternatively, the app has been updated to gracefully handle database errors.");
console.log("Even if the database throws an error, the app will continue to function using local cookies.");
console.log("\nTesting Supabase connection (REST API)...");
...
async function checkDb() {
  ...
  if (!url || !key) {
    console.log("❌ Could not find Supabase URL or Anon Key in .env.local.");
    return;
  }
  
  try {
    const res = await fetch(\`\${url}/rest/v1/user_settings?limit=1\`, {
      headers: {
        'apikey': key,
    ...
    if (res.ok) {
      ...
      console.log(\`❌ Failed to connect: HTTP \${res.status} \${res.statusText}\`);
    }
  } catch (err) {
