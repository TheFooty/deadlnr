...
import { ShieldCheck, Lock, Sparkles, Check, HelpCircle, ArrowLeft, Save, Loader2, Bot, Brain } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-[#080A0F] text-slate-100 flex flex-col font-sans">
      ...
      <main className="flex-1 max-w-3xl mx-auto w-full p-4 sm:p-6 md:p-8">
        ...
        {message && (
          <div
            className={`mb-6 rounded-2xl border p-4 text-sm font-semibold flex items-center gap-2 ${
              message.type === 'success'
                ? 'border-[#00E599]/40 bg-[#00E599]/10 text-[#00E599]'
                : 'border-[#FF0055]/40 bg-[#FF0055]/10 text-[#FF0055]'
            }`}
          >
            {message.type === 'success' ? <Check className="h-5 w-5 text-[#00E599]" /> : <HelpCircle className="h-5 w-5 text-[#FF0055]" />}
        ...
        <form onSubmit={handleSave} className="space-y-8">
          ...
          <div className="rounded-3xl border border-slate-800 bg-[#111622] p-6 shadow-xl backdrop-blur-xl card-tactile">
            <h2 className="text-lg font-bold text-white mb-1 flex items-center gap-2 font-display">
              <Sparkles className="h-5 w-5 text-[#FF3B00]" />
              <span>Default AI Assistant</span>
            </h2>
            <p className="text-xs text-slate-400 mb-5">
              Swiping right automatically copies assignment context and opens your chosen AI chat.
            </p>
                ...
                return (
                  <label
                    ...
                    className={`relative flex flex-col justify-between rounded-2xl border p-4 cursor-pointer transition-all ${
                      isSelected
                        ? 'border-[#FF3B00] bg-[#FF3B00]/10 shadow-lg shadow-[#FF3B00]/10'
                  ...
                  >
                    <div className="flex items-center justify-between mb-3">
                      <input
                        ...
                        className="sr-only"
                      />
                      <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-slate-900 text-[#FF3B00]">
                        {key === 'gemini' && <Sparkles className="h-4 w-4" />}
                        {key === 'chatgpt' && <Bot className="h-4 w-4" />}
                        {key === 'claude' && <Brain className="h-4 w-4" />}
                      </div>
                      <span className="text-[10px] font-mono font-bold uppercase tracking-wider px-2 py-0.5 rounded-full bg-slate-900 text-slate-300">
                        {provider.badge}
                      </span>
                    </div>
                    ...
                    <div>
                      ...
                      <p className="text-[11px] font-mono text-slate-400 truncate mt-0.5">{provider.url}</p>
                    </div>

                    {isSelected && (
                      <div className="absolute top-2 right-2 flex h-5 w-5 items-center justify-center rounded-full bg-[#FF3B00] text-slate-950">
          ...
          <div className="rounded-3xl border border-slate-800 bg-[#111622] p-6 shadow-xl backdrop-blur-xl card-tactile">
            <div className="flex items-center justify-between mb-1">
              <h2 className="text-lg font-bold text-white flex items-center gap-2 font-display">
                <Lock className="h-5 w-5 text-[#00E599]" />
                <span>Canvas Calendar Feed URL</span>
              </h2>
              {hasFeedUrl && (
                <span className="inline-flex items-center gap-1 text-xs font-mono font-bold text-[#00E599] bg-[#00E599]/10 px-2.5 py-1 rounded-full border border-[#00E599]/20">
            ...
            <div className="space-y-4">
              ...
              <div className="rounded-2xl border border-slate-800 bg-slate-950/80 p-4 text-xs text-slate-300 space-y-2">
              ...
              <div className="flex items-center gap-2 text-[11px] text-slate-400 bg-slate-900/40 p-3 rounded-xl border border-slate-800">
                <ShieldCheck className="h-4 w-4 text-[#00E599] shrink-0" />
                <span>
                  <strong>Privacy First:</strong> Your feed URL is encrypted using AES-256-GCM before saving and parsed server-side to prevent CORS issues.
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="flex justify-end">
            <button
              type="submit"
              disabled={saving}
              className="inline-flex items-center gap-2 rounded-2xl bg-[#FF3B00] hover:bg-[#FF3B00]/90 px-8 py-3.5 text-sm font-bold text-white shadow-xl shadow-[#FF3B00]/20 active:scale-95 transition-all disabled:opacity-50 font-display"
            >
