...
export async function POST(request: NextRequest) {
  try {
    ...
    // Determine current origin domain for redirect configuration
    const origin = request.headers.get('origin') || 'https://deadlnr-v.vercel.app';
    const redirectUrl = `${origin}/login`;

    // Trigger Supabase OTP auth flow with explicit domain redirect URL
          ...
          html: `
            <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto; padding: 24px; background-color: #080a0f; color: #ffffff; border-radius: 20px; border: 1px solid #1e293b;">
              ...
              <div style="background-color: #111622; border: 1px solid #00e599; border-radius: 16px; padding: 20px; text-align: center; margin-bottom: 24px;">
                <p style="color: #94a3b8; font-size: 12px; margin: 0 0 8px 0; text-transform: uppercase; font-family: monospace;">Verification Code Sent</p>
                <p style="color: #00e599; font-size: 14px; font-weight: 600; margin: 0;">Enter the verification code sent to your account on Deadlnr.</p>
              </div>
